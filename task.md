...
- 问题
  ...
  - ```
    ...
    缺少对应模块的加载，没有.o文件，有的函数和宏需要增加宏定义，具体参考`rk3399实践心得`
...
- 熟悉framebuffer框架
- 阅读正点原子驱动大全，再次从头学习linux驱动

#### 实践：

- 基于imx6ul的st7735s的spi协议framebuffer移植完毕
...
- 需要在内核上多下功夫，而且rk3399的售后问题没有处理好，交际能力有待提升
...
- 内核看了一部分《linux内核设计与实现》数据结构相关，kfifo\（双向）链表\映射\二叉树
- 数采方案的初步确认。

#### 拓展：

- 《linux驱动开发详解4.0内核》继续打基础
- 阅读uart部分源码，总结uart_driver&platform_driver特性，uart_driver面向应用层驱动，platform_driver面向硬件层驱动。
...
- 《linux驱动开发详解4.0内核》继续阅读
- 跟进一下数采的进度

#### 完成情况：

- 复习《linux驱动开发详解4.0内核》基础部分，了解inode和mknod指令的联系
- 数采的开发环境搭建及熟悉
